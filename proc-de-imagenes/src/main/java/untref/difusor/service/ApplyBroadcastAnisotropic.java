...
public class ApplyBroadcastAnisotropic {
	public WritableImage difussion(Image image, int numberRepeat,double sigma) {
		...
		Anisotropic anisotropic = new Anisotropic();
		...
		for (int h = 0; h < numberRepeat; h++) {
			for (int i = 0; i < (int) image.getWidth(); i++) {
				for (int j = 0; j < (int) image.getHeight(); j++) {
					...
					newGrey = anisotropic.valueDiffusion(grey, northDerivative, southDerivative, eastDerivative,
							westDerivative, sigma);
