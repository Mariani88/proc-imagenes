...
public class HoughCircleDetector {
	...
	public void setConfigHoughCircle(int radius, float threshold) {
		...
		circleConfig.setHOUGH_MIN_RADIUS(radius);
		if (threshold==30f)
			threshold=1.3f;
		circleConfig.setHOUGH_CIRCLE_DETECTION_THRESHOLD(threshold);
	...
	public BufferedImage detectCircles(Image image) {
		...
		BufferedImage originalImage = this
				.cannyImage(image); /*
		...
		int minRadius =this.circleConfig.getHOUGH_MIN_RADIUS(); 
		int maxRadius = this.circleConfig.getHOUGH_MAX_RADIUS(); 
		int radiusStep = this.circleConfig.getHOUGH_RADIUS_STEP(); 
		int numRFrames = (maxRadius - minRadius) / radiusStep;
		int[][][] accumulator = new int[3][img.getWidth()][img.getHeight()];
		float thresh =this.circleConfig.getHOUGH_CIRCLE_DETECTION_THRESHOLD(); 
	...
	//	System.out.println("CIRCLES FOUND:_____" + circles.size());
		...
		for (int i = 0; i < Math.min(circles.size(), this.circleConfig.getHOUGH_MAX_EXPECTED_CIRCLES()); i++) {
	...
	private int[][] smoothFrame(int[][] accumulator) {
		double[][] gaussian = this.circleConfig.getGAUSSIAN_5();
