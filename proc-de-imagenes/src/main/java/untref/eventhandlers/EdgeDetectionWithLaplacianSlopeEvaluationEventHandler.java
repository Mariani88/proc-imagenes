...
public class EdgeDetectionWithLaplacianSlopeEvaluationEventHandler implements EventHandler<ActionEvent> {
	...
	public EdgeDetectionWithLaplacianSlopeEvaluationEventHandler(ImageView imageView, ImageView imageResultView, EdgeDetectionService edgeDetectionService) {
	...
	public void handle(ActionEvent event) {
		...
		new ParametersWindowsFactory().create(Arrays.asList(maxSlopePercent, maxSlopePercentValue), event1 -> {
			...
			Image imageWithEdges = edgeDetectionService.detectEdge(imageView.getImage(), new LaplacianWithSlopeEvaluationOperator(maxSlopePercent1));
